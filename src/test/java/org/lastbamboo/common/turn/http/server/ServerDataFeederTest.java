...
public class ServerDataFeederTest {
    ...
    private final AtomicBoolean m_socketServingFailed = new AtomicBoolean(false);
    ...
    public void testFeedServer() throws Exception {
        ...
        final IoSession session = new IoSessionStub() {
            ...
            public WriteFuture write(final Object message) {
                final SendIndication si = (SendIndication) message;
            ...
            private void addRaw(final Collection<ByteBuffer> bufs,
                ...
                // We no longer use TcpFrames, so the below is not necessary.
                //final byte[] noFrame = ArrayUtils.subarray(rawData, 2,
                //        rawData.length);

                totalDataReceived.addAndGet(rawData.length);
                bufs.add(ByteBuffer.wrap(rawData));
        ...
        assertEquals("Did not receive all the data we sent",
    ...
    private void verifyData(final Collection<ByteBuffer> value) {
        ...
        for (int i = 0; i < NUM_MESSAGES_PER_HOST; i++) {
            ...
            final byte[] subArray = ArrayUtils.subarray(data, index, index
                    + DATA.length);
    ...
    private InetSocketAddress startThreadedServer() throws UnknownHostException {
        final InetSocketAddress serverAddress = new InetSocketAddress(
                NetworkUtils.getLocalHost(), 7896);
        final Runnable runner = new Runnable() {
            public void run() {
                try {
                    startServer(serverAddress);
                } catch (final Throwable t) {
                    fail("Bad, bad server: " + ThreadUtils.dumpStack());
                }
            }
        };
        final Thread turnThread = new Thread(runner,
                "TURN-Test-Data-Server-Thread");
    ...
    private void startServer(final InetSocketAddress serverAddress)
            throws Exception {
        ...
        while (!this.m_dataServerSocket.isClosed()) {
            ...
            final Thread serverSocketThread = new Thread(serverSocketRunner,
                    "Server-Socket-Serving-Thread");
    ...
    private void serveDataSocket(final Socket client) throws IOException {
        ...
        while (!m_stopDataServer.get()) {
            ...
            if (!Arrays.equals(DATA, request)) {
                ...
                m_log.debug("Data server has validated "
                        + m_dataServerMessagesValidated + " messages...");
            }
